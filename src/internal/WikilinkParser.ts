import type { PageDirectoryService } from "./find-page.js";
...
export default class WikilinkParser {
   ...
   * @param { Map } linkCache
   */
  constructor(public opts: any, public deadLinks: any, public linkCache: any) {
  }

  /**
   * Parses a single WikiLink into the link object understood by the Interlinker.
   */
  parseSingle(link: string, pageDirectory: PageDirectoryService, filePathStem: string | undefined = undefined): WikilinkMeta {
    ...
    if (page) {
      if (foundByAlias) {
        ...
        meta.title = String(page.data.title);
      }
      meta.href = page.url;
  ...
  parseMultiple(links: string[], pageDirectory: PageDirectoryService, filePathStem: string| undefined): WikilinkMeta[] {
   ...
   * parsed result.
   */
  find(document: string, pageDirectory: PageDirectoryService, filePathStem: string | undefined): WikilinkMeta[] {
